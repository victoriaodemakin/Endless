...
function myticket_events_cgb_block_assets() { // phpcs:ignore
	...
	wp_enqueue_script( 'myticket-events-script2', plugins_url( 'listing-02/script.js', __FILE__ ), array('jquery'), MYTICKET_VERSION );
	wp_enqueue_script( 'myticket-events-script3', plugins_url( 'listing-03/script.js', __FILE__ ), array('jquery'), MYTICKET_VERSION  );
	wp_enqueue_script( 'myticket-events-script4', plugins_url( 'listing-04/script.js', __FILE__ ), array('jquery'), MYTICKET_VERSION  );
	wp_enqueue_script( 'myticket-events-script5', plugins_url( 'listing-05/script.js', __FILE__ ), array('jquery'), MYTICKET_VERSION  );

	// jQuery calendar
	if ( '1' == get_theme_mod('myticket_calendar', '0') ){

		wp_enqueue_script( 'myticket-events-calendar', plugins_url( 'dist/assets/jquery-ui.min.js', dirname( __FILE__ ) ), array('jquery'), MYTICKET_VERSION  );
		wp_enqueue_style( 'myticket-events-calendar', plugins_url( 'dist/assets/jquery-ui.min.css', dirname( __FILE__ ) ) );
    ...
    if ( has_blocks( $post_object ) ) {
        ...
        foreach ($blocks as $block) {
            if ($block['blockName'] == 'myticket-events/listing-01') {
				...
				wp_enqueue_script( 'bootstrap-slider', plugins_url( '/dist/assets/bootstrap-slider.min.js', dirname( __FILE__ ) ), array('jquery'), MYTICKET_VERSION  );
				wp_enqueue_script( 'myticket-events-script', plugins_url( 'listing-01/script.js', __FILE__ ), array('jquery'), MYTICKET_VERSION  );
            }
        }
    }
}
